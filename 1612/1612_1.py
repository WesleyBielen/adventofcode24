...
firstiter=True
lowest_score=0
lowest_score_per_plot=None
sys.setrecursionlimit(10000000) 
def main():
    global startpos, endpos, lowest_score_per_plot
    ...
    for i in range(len(matrix)):
        for ii in range(len(matrix[i])):
            ...
            if 'E' == matrix[i][ii]:
                endpos=(i,ii)
    lowest_score_per_plot = [['-1' for _ in row] for row in matrix]
...
def eval(pos, matrix, previous_direction, direction, totalscore, plot_traveled):
    global firstiter, lowest_score, lowest_score_per_plot
    ...
    if not int(lowest_score_per_plot[pos[0]][pos[1]]) == -1 and totalscore > int(lowest_score_per_plot[pos[0]][pos[1]]):
        return
    else:
        lowest_score_per_plot[pos[0]][pos[1]] = totalscore
    ...
    if not matrix[pos[0]+1][pos[1]] == "#":
        eval((pos[0]+1,pos[1]), matrix, direction, 'S', totalscore, plot_traveled[:already_traveled])

    if not matrix[pos[0]][pos[1]-1] == "#":
        eval((pos[0],pos[1]-1), matrix, direction,'W', totalscore, plot_traveled[:already_traveled])

    if not matrix[pos[0]-1][pos[1]] == "#":
        eval((pos[0]-1,pos[1]), matrix, direction, 'N', totalscore, plot_traveled[:already_traveled])
